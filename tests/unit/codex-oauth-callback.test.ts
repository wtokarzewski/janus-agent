...
function callBack(port: number, path: string, agent: http.Agent): Promise<void> {
...
describe('codex OAuth callback server', () => {
  it('releases the browser keep-alive socket after the callback', async () => {
    ...
    const { code, server } = startCallbackServer('state-abc', 0);
    ...
    try {
      await callBack(port, '/auth/callback?code=the-code&state=state-abc', agent);
  ...
  it('rejects and shuts down on a state mismatch', async () => {
    ...
    const agent = new http.Agent({ keepAlive: true });
    try {
      await callBack(port, '/auth/callback?code=the-code&state=wrong', agent);
