...
export class CronTool implements ContextualTool {
  ...
  parameters = {
    ...
    properties: {
      ...
      chat_id: {
        type: 'string',
        description: 'Target chat ID for this job. Defaults to the current conversation\'s chat — the reminder lands wherever this request is coming from. Specify explicitly ONLY when the reminder should land in a different chat than where the conversation is happening, and verify with the user first that the different chat is intended. When asking the user to confirm a different chat, refer to it by NAME from <your_chats> (e.g. "Dieta", "Rodzina"), never by raw chat ID — raw IDs are meaningless to the user.',
      },
      not_before: {
        type: 'string',
  ...
  async execute(args: Record<string, unknown>, reqCtx?: RequestContext): Promise<string> {
    ...
    switch (action) {
      ...
      case 'add': {
        ...
        // Resolve effective chatId. If the caller did not pass chat_id explicitly,
        // default to the current conversation's chatId so the reminder lands in
        // the same chat the user is talking on. Skip the default when the current
        // context is a synthetic cron/system trigger (e.g. one cron creating
        // another) — those have no real user-facing chat to inherit.
        let effectiveChatId: string | undefined = args.chat_id ? String(args.chat_id) : undefined;
        let chatIdDefaulted = false;
        if (!effectiveChatId && reqCtx?.chatId && !isSyntheticChatId(reqCtx.chatId)) {
          effectiveChatId = reqCtx.chatId;
          chatIdDefaulted = true;
        }
        ...
        const result: Record<string, unknown> = JSON.parse(JSON.stringify(job));
        if (chatIdDefaulted) {
          result._chatIdDefaulted = `chat_id inherited from the current conversation (${effectiveChatId}). Pass chat_id explicitly to override.`;
        }
      ...
      case 'runs': {
        ...
        // Wrap the run list with an explainer because the raw fields are
        // ambiguous: `durationMs` measures how long it took to publish the
        // trigger to the bus (almost always tiny — single-digit ms), NOT how
        // long the agent that processed the trigger ran. `status: "ok"` means
        // the trigger fired successfully, NOT that the user received a
        // message. Without this note, callers (LLMs included) misread a 1ms
        // duration as "nothing happened."
        const result = {
          _note: 'durationMs = time to publish the trigger to the bus, NOT agent runtime. The agent that handles the trigger runs asynchronously and its work is not reflected in these numbers. status="ok" means the cron triggered successfully — it does NOT confirm the user received a message. To verify delivery, check the relevant chat history or ask the user.',
          runs,
        };
        return JSON.stringify(result, null, 2);
      }
...
/**
 * A chatId is "synthetic" when it was generated by Janus to give a cron/system
 * trigger a session of its own (e.g. `cron:<jobId>`). Those are internal
 * routing tokens, not user-facing chats — so they must never be inherited
 * when a new job's chat_id defaults from reqCtx.
 */
function isSyntheticChatId(chatId: string): boolean {
  return chatId.startsWith('cron:') || chatId.startsWith('system:');
}
