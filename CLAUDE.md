...
Janus is a universal AI agent. CLI + Telegram + Browser Operator, ~13,400 lines TypeScript.
...
- `agent/` — Agent loop (LLM iteration with tool calls, `stop()` for mid-task cancellation, MAX_ITERATIONS=200 hard safety limit, cross-tool loop detection with 6-call pattern window, proactive context overflow detection at 90%/95% with pruneOldToolResults), subagent spawning (partial progress on timeout), SubagentRegistry (cancel/cancelAll)
