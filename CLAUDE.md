...
846 tests across 80 test files: unit (agent-resolver, anthropic-oauth, async-queue, browser-tool, circuit-breaker, codex-oauth, codex-oauth-callback, config-schema, context-builder, cron-service, cron-tool, error-classification, exec-tool, gate-routing, heartbeat-parser, heartbeat-resync, instance-lock, invite, lane-watchdog, learner, mcp-server, memory-index, model-drift, notify-owner, oauth-refresh-lock, pattern-gate, pkce, provider-registry, sanitize, sdk-utils, self-update-tool, provider-command, session-lock, update-stamp, setup, shell, test-run-outcome, skill-loading, stop-command, streaming, system-message, telegram-channel, token-counting, token-store, tool-registry, user-resolver, validate-path, vector-search, version-utils, voice-transcribe, web-tools) + integration (agent-loop with mock LLM). CI runs on push/PR via GitHub ActionsPrior: 813 tests across 76 test files: unit (agent-resolver, anthropic-oauth, async-queue, browser-tool, circuit-breaker, codex-oauth, codex-oauth-callback, config-schema, notify-owner, oauth-refresh-lock, error-classification, model-catalog, context-builder, cron-service, cron-tool, error-classification, exec-tool, gate-routing, heartbeat-parser, heartbeat-resync, instance-lock, invite, lane-watchdog, learner, mcp-server, memory-index, pattern-gate, pkce, provider-registry, sanitize, sdk-utils, self-update-tool, session-lock, setup, shell, skill-loading, stop-command, streaming, system-message, telegram-channel, token-counting, token-store, tool-registry, user-resolver, validate-path, vector-search, version-utils, voice-transcribe, web-tools) + integration (agent-loop with mock LLM). CI runs on push/PR via GitHub Actions.
