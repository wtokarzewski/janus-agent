...
- **Setup wizard model lists** — Anthropic subscription menu offers sonnet 5 / opus 5 / haiku 4.5 / fable 5; Codex menu offers GPT-5.6 Terra (default) / Sol / Luna + gpt-5.5. Live lists fetched from the provider API are reduced to the current generation by `keepLatestPerFamily()` — one entry per model family, highest version, siblings of that version kept. Defaults sit on the balanced tier, not the flagship. API-key providers fetch the live list from the provider API; these lists are the offline fallback and the prompt default.
