...
- **Update announcement with build stamp** — after a successful update, manual or automatic, every configured user gets a DM on the next gateway start: `🔄 Janus zaktualizowany i zrestartowany` + `v0.15.0 • 6e1ed1f • 2026-08-05 21:14` + the list of commits that landed. The stamp (version from `package.json`, short hash and commit time from git) is read at startup, so it describes the code actually running; a tarball install with no repository shows the version alone. `npm start -- update` now writes the same `.janus/.update-complete` marker `self_update` did, so both paths announce identically. Group chats are excluded — a technical notice belongs to people, not rooms.
