...
- **Model drift check on update** — `npm start -- update` compares the models in `llm.slots` against each provider's live list and warns about any the provider no longer serves, suggesting the newest one available. Advisory only: nothing is rewritten, and a provider that can't be queried (subscription tokens have no listing endpoint) is skipped silently.
- **Model catalog** — current models only, no historical entries. `claude-agent` alias map: `opus`→`claude-opus-5-5`, `sonnet`→`claude-sonnet-5`, `haiku`→`claude-haiku-4-5-20251001`, `fable`→`claude-fable-5-1`, plus explicit pins `opus-5-5`/`sonnet-5`/`fable-5-1`. Superseded releases stay reachable by full model ID. Default slot ships `claude-sonnet-5`. `anthropic` API-key provider takes full model IDs. `modelRejectsSamplingParams()` omits `temperature`/`top_p`/`top_k` for every model that rejects them (Opus 5.5/5/4.8/4.7, Sonnet 5, Fable, Mythos) — a compatibility guard, kept for superseded IDs too.
- **exec reports a timeout without waiting for teardown** — the tool used to resolve only on the child's `close` event, so the call hung until the OS finished killing the tree: on Unix through the SIGKILL grace period, on Windows indefinitely when a grandchild survives `taskkill` holding the inherited stdout pipe. It now reports at the timeout with whatever output arrived, and kills best-effort in the background.
- **Update never reverts on an unrun suite** — `update` and `self_update` tell a real test failure (non-zero exit) from a runner that never finished (timeout, output-buffer overflow, missing binary) via `classifyTestRun()`. Only the former reverts; the latter keeps the update and reports. Revert targets the pre-pull commit, not `HEAD~1`, so a multi-commit pull rolls back fully. Test timeout 2 min → 10 min and `maxBuffer` 32 MB, because Windows with a cold cache runs the suite far slower than CI.
- **`/provider` — manual provider switch** — numbered menu of configured providers with role (default/fallback), model, which one is currently serving and which the circuit breaker has demoted. Reply `/provider 2` to send traffic there, `/provider 0` back to automatic. Runtime pin on the registry: applies to the next message, no restart, not written to config, and it overrides a breaker demotion because the operator asked for it. Other providers stay behind the pin as fallback. On Telegram any configured user may use it (it sits behind the allowlist, so a stranger messaging the bot cannot); in CLI it is always available.
- **Two-stage release** — `Release (prepare)` bumps the version and dates the CHANGELOG on a branch and opens a pull request; `Release (publish)` runs when that lands on main, and if the version has no tag yet it builds the tarball, tags and publishes the GitHub release. The old single job pushed the bump straight to main, which the branch ruleset rejects for everyone including the Actions bot — every release failed at the last step and had to be finished by hand.
- **`/restart` and `/shutdown now`** — take Janus down from chat, and bring it back. `/restart` flushes sessions and hands over to the same detached worker the updates use, in restart mode (no code changes), so it comes back on its own. `/shutdown` requires the explicit `now`: it is the one chat command nobody can undo from chat, since only someone at the machine can start Janus again. Both release the instance lock before exiting, so the next start does not have to take over a stale one. Both flush sessions first, so the conversation survives either way; a restart worker that fails to start leaves the current instance running and says so.
- **`/help` on Telegram** — lists the built-in commands (`/help`, `/provider`, `/model`, `/stop`, `/whoami`) with one line each. The CLI has had a help screen from the start; on Telegram there was no way to discover the commands existed. The list lives next to the handlers, so adding a command and documenting it is one edit.
- **`JANUS_NO_AUTO_UPDATE=1`** — emergency brake: the scheduled update check is not registered while it is set (logged loudly at startup). Updates asked for in chat still work, because those are deliberate. For the moment when something is already broken and the last thing wanted is the cron pulling new code into the incident.
- **`autoUpdate.restartMode`** — `respawn` (default) starts a replacement process; `exit` just stops after installing, leaving the restart to a service manager (systemd, NSSM, Docker restart policy). Under supervision `exit` is the reliable option: a dying process is a poor place to start its successor.
- **Updates run in a detached worker** — `self_update` no longer updates in place. It confirms there is something to pull, flushes sessions, starts an independent `update-worker` process, verifies it is alive and exits. The worker waits for the gateway's instance lock to clear (60s cap — it refuses to swap files under a live process), runs the same `update` pipeline as the CLI, and starts a fresh gateway **whether the update succeeded or failed**, replaying the flags it was launched with (`gateway --token-debug` comes back with the flag, carried through `JANUS_RESTART_ARGV`). Nothing depends on a dying process starting its successor, which is what kept breaking the restart. If the worker fails to start, the current version keeps running and says so.
- **Self-restart detaches on Windows** — the chain is PowerShell → npm → node → child, and npm tears its process tree down when the parent exits, so a child sharing that console died the moment the old process left, even after reporting a healthy start. The replacement now spawns detached with its own stdio (the file log covers output).
- **Self-restart preserves the runtime flags** — `self_update` respawns with `process.execArgv` in front of the arguments. tsx injects its loader there, so restarting with `argv.slice(1)` alone produced a plain `node src/index.ts` that died on the first import: the update applied, the process exited and nothing came back. If the replacement dies within 3s the parent stays alive on the old code and says so, rather than leaving nothing running.
- **Update announcement with build stamp** — after a successful update, manual or automatic, every configured user gets a DM on the next gateway start: `🔄 Janus zaktualizowany i zrestartowany` + `v0.15.0 • 6e1ed1f • 2026-08-05 21:14` + up to five commit subjects (`…and N more` beyond that) — subjects, not the `git pull` diffstat nobody reads in a chat window. The stamp (version from `package.json`, short hash and commit time from git) is read at startup, so it describes the code actually running; a tarball install with no repository shows the version alone. `npm start -- update` now writes the same `.janus/.update-complete` marker `self_update` did, so both paths announce identically. Group chats are excluded — a technical notice belongs to people, not rooms.
- **Operator commands sit behind the allowlist** — `/provider` and `/model` used to be handled *before* the allowlist check, so anyone who messaged the bot could change the model for every chat on the instance. Both now run after it: any configured user may switch, an unknown sender is ignored as usual.
- **Setup verifies both providers** — after saving, the wizard pings the primary *and* the fallback with a one-token request. The fallback is the provider that runs when the primary is down, so an untested one turns a bad key into a surprise at the worst moment. A failure prints and does not abort the wizard.
- **Setup wizard model lists** — Anthropic subscription menu offers sonnet 5 / opus 5.5 / haiku 4.5 / fable 5.1; Codex menu offers GPT-6 Sol (default) / Astra / Luna. Defaults sit on the middle tier, not the flagship. Subscription (OAuth) requests identify as `claude-cli/2.1.280` via the single `CLAUDE_CLI_USER_AGENT` constant. API-key providers fetch the live list from the provider API; these lists are the offline fallback and the prompt default.
